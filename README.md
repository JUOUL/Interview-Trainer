...
**2027 秋招**
- 首页右上角「2027秋招」入口进入秋招看板（路由 `/recruitment`）
- 横向招聘流程看板（待关注 → 已开放 → 已投递 → 笔试/测评 → 面试 → Offer → 结束），可切换表格视图
- 顶部统计：企业总数 / 已开放 / 已投递 / 笔试测评 / 面试 / Offer / 七天内截止 / 七天内待办
- 支持按名称搜索，按类别、优先级、状态、目标岗位筛选，一键清除
- P0 最醒目，截止过期 / 临近截止 / 待办临近自动提醒
- 点击卡片查看完整详情；状态为「面试」的企业可一键跳转到面试训练页
- 唯一数据源：`public/recruitment-companies.json`，修改后刷新页面即生效
- 数据由 Claude 按自然语言指令维护，规则见 `CLAUDE.md`
